...
#   zerodce DWC + p-shared
    ...
    def enhance(self, x,x_r):
        ...
        x_cpu = x.cpu().detach()
        xr_cpu = x_r.cpu().detach()
        
        for i in range(8):
            x_sq = mytensor.tensor_multiply(x_cpu.numpy(),x_cpu.numpy())
            x_sq = torch.from_numpy(x_sq).to(device)

            x_sub = x_sq - x

            x_alpha = mytensor.tensor_multiply(xr_cpu.numpy(), x_sub.numpy())
